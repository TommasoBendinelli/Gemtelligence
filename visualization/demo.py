...
def main():
    ...
    if task == "Origin":
        ...
        cfg.sources["ed"] = {
            "name": "ed",
            "cancel_out_threshold": True,
            "num_columns": 26,
            "val_method": "max",
            "is_log": False,
            "is_z_normalized": False,
        }
        cfg.sources["icp"] = {
            "name": "icp",
            "include_strongly_mislabelled_data": "???",
            "include_additional_info": True,
            "include_additional_info2": False,
            "cancel_out_threshold": True,
            "include_ED": False,
            "data_feeding": "single",
            "calibrate": False,
            "include_reference_data": -1,
            "unsupervised_idea": False,
        ...
        if stone_type == "Client Stones":
            pre_candidate_ids = sapphire_data["val"].index
        ...
        _, pre_candidate_ids = utils.filter_data_for_task(
            sapphire_data, "treatment", years=False
        )
    ...
    # The model and the data released are not from the same split, so we can select the entire subset
    candidate_ids = pre_candidate_ids
    ...
    if task == "Origin":
        ...
        for col in sapphire_data["icp"].columns:
            sapphire_data["icp"][col] = sapphire_data["icp"][col].apply(
                pd.to_numeric, errors="coerce"
            )
            sapphire_data["icp"] = sapphire_data["icp"].loc[
                ~(sapphire_data["icp"][col] < 0)
            ]

        sapphire_data["icp"] = sapphire_data["icp"].loc[
            ~(sapphire_data["icp"]["Al27"] < 500000)
        ]
        sapphire_data["icp"] = sapphire_data["icp"].loc[
            ~(sapphire_data["icp"]["Nd146"] > 10)
        ]
    ...
    sources = st.multiselect(
        "Select Sources for the Network", all_sources, default_sources
    )
    ...
    for source in sources:
        ...
        if source == "xrf":
            if type(curr) == pd.DataFrame:
                chosen = st.selectbox(
                    "Which measurement do you want to visualize for ED?",
                    list(range(len(curr))),
                )
            ...
            for element in curr.index:
                if all_ed_data[element].min() != all_ed_data[element].max():
                    dimensions.append(
                        dict(
                            range=[
                                all_ed_data[element].min(),
                                all_ed_data[element].max(),
                            ],
                            label=element,
                            values=[curr[element]],
                        )
                    )
            fig = go.Figure(data=go.Parcoords(line_color="blue", dimensions=dimensions))
            ...
            if type(curr) == pd.DataFrame:
                chosen = st.selectbox(
                    "Which measurement do you want to visualize for ICP?",
                    list(range(len(curr))),
                )
                curr = curr.iloc[chosen]
            ...
            for element in curr.index:
                dimensions.append(
                    dict(
                        range=[
                            all_icp_data[element].min(),
                            all_icp_data[element].max(),
                        ],
                        label=element,
                        values=[curr[element]],
                    )
            ...
            if type(curr) == pd.DataFrame:
                chosen = st.selectbox(
                    "Which measurement do you want to visualize for UV?",
                    list(range(len(curr))),
                )
                curr = curr.iloc[chosen]
            ...
            fig.update_layout(
                title="", xaxis_title="Wavelength", yaxis_title="Intensity"
            )
    ...
    if task == "Origin":
        entry["ed"] = pd.DataFrame(
            entry["ed"],
            index=[test_stone_selected],
            columns=[f"ed_{idx}" for idx in range(entry["ed"].shape[1])],
        )
        entry["icp"] = pd.DataFrame(
            entry["icp"],
            index=[test_stone_selected],
            columns=[f"icp_{idx}" for idx in range(entry["icp"].shape[1])],
        )
        entry["uv"] = pd.DataFrame(
            entry["uv"],
            index=[test_stone_selected],
            columns=[f"uv_{idx}" for idx in range(entry["uv"].shape[1])],
        )
    elif task == "Treatment":
        entry["uv"] = pd.DataFrame(
            entry["uv"],
        ...
        if len(entry["ftir"].shape) > 2:
            entry["ftir"] = entry["ftir"][:, 1, :]  # Quick fix for demo issue
        entry["ftir"] = pd.DataFrame(
            entry["ftir"],
            index=[test_stone_selected],
            columns=[f"ftir_{idx}" for idx in range(entry["ftir"].shape[1])],
        )
    ...
    if task == "Origin":
        st.markdown(
            f"**Ground truth**: {sapphire_data['val'].loc[test_stone_selected]['FinalOrigin']}"
        )
    elif task == "Treatment":
        st.markdown(
            f"**Ground truth**: {sapphire_data['val'].loc[test_stone_selected]['Heat Treatment Value']}"
        )
    else:
