...
def final_validation_processing(entries, sources):
    ...
    df_tot.columns = pd.MultiIndex.from_tuples(
        [(k, column) for k, df in entries.items() for column in df]
    )
    ...
    if "uv" in sources:
        ...
        tmp = uv_values
        uv_values = np.reshape(
            uv_values, (uv_values.shape[0], 2, uv_values.shape[1] // 2)
        )
...
def get_uv(ID, uv_df, is_training, prob_to_ignore=0):
    ...
    if is_training:
        if ID in uv_df.index and random.random() > prob_to_ignore:
            ...
            uv_values = np.array(
                [0.0] * len(uv_df[["A", "B"]].columns)
            )  # pd.DataFrame({x:[0] for x in uv_df.columns}, index=[ID])
        ...
        if ID in uv_df.index and random.random() > prob_to_ignore:
            ...
            uv_df = pd.DataFrame(
                {f"uv_{x}": [0] for x in range(len(uv_df[["A", "B"]].columns))},
                index=[ID],
            )  # np.array([0] *  len(uv_df[["A","B"]].columns))#pd.DataFrame({x:[0] for x in uv_df.columns}, index=[ID])
        return uv_df
...
def get_ed(
    ...
    if is_training:
        ...
        if ID in ed_df.index and random.random() > prob_to_ignore:
            ...
            ed_df = pd.DataFrame(
                {"ed_" + str(x): 0 for x in range(len(ed_df["Concentration"].columns))},
                index=[ID],
            )
        tmp = ed_df
...
def get_icp(ID, icp_df, is_training, prob_to_ignore=0, is_log=False):
    ...
    if is_training:
        ...
        if ID in icp_df.index and random.random() > prob_to_ignore:
            ...
            icp_df = pd.DataFrame(
                {"icp_" + str(x): 0 for x in range(len(icp_df.columns))}, index=[ID]
            )
        tmp = icp_df
    ...
    if is_training:
        ...
        if ID in ftir_df.index and random.random() > prob_to_ignore:
            ...
            ftir_df = pd.DataFrame(
                {f"ftir_{x}": [0.0] for x in range(len(ftir_df.columns))}, index=[ID]
            )  # np.array([0] *  len(uv_df[["A","B"]].columns))#pd.DataFrame({x:[0] for x in uv_df.columns}, index=[ID])
        return ftir_df
...
class CombinedDataset(data.Dataset):
    def __init__(self, data, cfg, mode, pretraining=False, mean=None, std=None):
        assert (
            pretraining == False
        )  # We don't do any pretraining, the argument is just passed for compatibility
        ...
        for source in cfg.sources.keys():
            if (
                "is_z_normalized" in cfg.sources[source]
                and cfg.sources[source].is_z_normalized
            ):
                if source == "ed":
        ...
        self.sources = set(
            cfg.sources.keys()
        )  # The data source used for the current experiment
    ...
    def __getitem__(self, index):
        ...
        if self.cfg.target in ("origin", "pre_origin"):
            ...
            raise NotImplementedError(
                "Target {} not implemented".format(self.cfg.target)
            )
    ...
    def get_training_sample(self, ID):
        ...
        assert any(
            [ID in self.data[x].index for x in self.sources]
        ), "ID {} not found in any of the sources. Bug Somewhere!".format(ID)
        entries = {}
        if "uv" in self.sources:
            uv = get_uv(
                ...
                self.data["uv"],
                is_training=True,
                prob_to_ignore=self.cfg.method.prob_to_drop,
            )
            entries["uv"] = uv
        if "ed" in self.sources:
            ed = get_ed(
                ...
                is_training=True,
                prob_to_ignore=self.cfg.method.prob_to_drop,
                is_log=self.cfg.sources.ed.is_log,
                is_z_normalized=self.cfg.sources.ed.is_z_normalized,
                mean=self.mean,
                std=self.std,
            )
            entries["ed"] = ed
        ...
        if "icp" in self.sources:
            icp = get_icp(
                ...
                is_training=True,
                prob_to_ignore=self.cfg.method.prob_to_drop,
                is_log=self.cfg.sources.icp.is_log,
            )
            entries["icp"] = icp
    ...
    def get_validation_sample(self, ID):
        ...
        if "ftir" in self.sources:
            ...
            entries["ftir"] = (
                ...
                .drop_duplicates()
                .iloc[:20]
            )  # We only use the first 50 ftir values to not go out of memory
        if "icp" in self.sources:
...
class DataModule(pl.LightningDataModule):
    def __init__(
        ...
        assert not learning.utils.obtain_all_stones_available_in_sources(
            self.train_data
        ) & learning.utils.obtain_all_stones_available_in_sources(self.val_data)
    ...
    def custom_collate_val(self, x):
        ...
        assert (
            len(x) == len_batch
        )  # Batch consists of only one sample in Validation and Test
        inputs = x[0][0]
        y = x[0][1]
        y_sec = x[0][2]
        ID = x[0][3]
        ...
        y_sec = torch.stack(
            [torch.tensor(int(y_sec)) for _ in range(number_of_samples)]
        )
