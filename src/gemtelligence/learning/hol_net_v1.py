...
def return_first_batch_from_model_path(model_path,is_raw=False):
    first_batch_path_test_one = Path(hydra.utils.to_absolute_path(Path(model_path).parent / "0" / "first_batch.pkl"))
    second_batch_path_test_second = Path(hydra.utils.to_absolute_path(Path(model_path).parent.parent.parent.parent / "first_batch.pkl"))
    ...
    def __init__(self, cfg=None, first_batch=None):
        ...
        if "uv" in cfg.sources.keys():
            ...
            if not cfg.method.all_saint:
                self.uv_model = ResNet(cfg=cfg.method.model.resnet, input_length=cfg.sources.uv.input_dim, input_channels=input_channels)
            self.uv_reductor = nn.Conv1d(self.uv_model.z_inner_dim.shape[1], 5, kernel_size=1)
        ...
        if "ftir" in cfg.sources.keys():
            ...
            self.ftir_model = ResNet(cfg=cfg.method.model.resnet, input_length=cfg.sources.ftir.input_dim, input_channels=input_channels) 
            stride = 1
            channels = 1
            self.ftir_reductor = nn.Conv1d(self.ftir_model.z_inner_dim.shape[1], channels, kernel_size=1, stride=stride)
        ...
        if "ed" in cfg.sources.keys() or "icp" in cfg.sources.keys():
            ...
            assert not (("tab_transformer" in cfg.method.model) & ("saint" in cfg.method.model))
            
            if "saint" in cfg.method.model:
                if "icp" in cfg.sources.keys() and "ed" in cfg.sources.keys():  
                    num_columns = cfg.sources.icp.num_columns + cfg.sources.ed.num_columns
                ...
                self.ed_model = SAINT(categories = tuple(np.array([1]).astype(int)), num_continuous = num_columns,
                            dim = cfg.method.model.saint.embedding_size, dim_out = 1,
                            depth = cfg.method.model.saint.transformer_depth,  heads = cfg.method.model.saint.attention_heads,
                            attn_dropout = cfg.method.model.saint.attention_dropout,ff_dropout = cfg.method.model.saint.ff_dropout, 
                            mlp_hidden_mults = (4, 2), cont_embeddings = cfg.method.model.saint.cont_embeddings, attentiontype = cfg.method.model.saint.attentiontype,
                            final_mlp_style = cfg.method.model.saint.final_mlp_style, y_dim = cfg.class_len, cfg=cfg)
        ...
        self.linear_classifier = nn.Linear(sum(self.end_dims.values()), self.number_of_classes+ cfg.secondary_class_len)
        self.lr = cfg.method.lr


    def model(self,x):
        return self.forward(x, is_aldread_padded=True)

   
    def forward(self,x, is_aldread_padded=False):
        ...
        # Ugly hack only for capsule 
            ...
            if "ftir" in self.cfg.sources:
                if x_method.shape[2] == self.cfg.sources.ftir.input_dim:
                    ...
                    x_ftir = torch.zeros(x_method.shape[0],1,self.cfg.sources.ftir.input_dim, device=self.device)
        ...
        for source in keys:
            if source in ["uv","ftir"]:
                ...
                if "saint" in self.cfg.method.model:
                    if is_aldread_padded == True:
                        _, x[source] = self.source_encoder[source](x, padding=None, device=self.device)
                    else:
                        _, x[source] = self.source_encoder[source](x, padding=self.first_batch, device=self.device)
                ...
                if "saint" in self.cfg.method.model:
                    if is_aldread_padded == True:
                        _, x[source] = self.source_encoder["ed"](x, padding=None, device=self.device)
                    else:
                        _, x[source] = self.source_encoder["ed"](x, padding=self.first_batch, device=self.device)
                    # x[source] is 26 element, make it to be 32
