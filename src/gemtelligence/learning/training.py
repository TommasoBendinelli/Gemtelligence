        ...
        for entry in to_check_against:
            ...
            for key in keys:
                if (stone in df_dict[key].index) and (entry in df_dict[key].index):
                    ...
                    stone_entry = stone_entry.sort_values(
                        by=[stone_entry.columns[0], stone_entry.columns[1]]
                    )

                    to_check_against_entry = df_dict[key].loc[entry:entry]
                    to_check_against_entry = to_check_against_entry.sort_values(
                        by=[
                            to_check_against_entry.columns[0],
                            to_check_against_entry.columns[1],
                        ]
                    )
                ...
                elif (stone in df_dict[key].index) and (
                    entry not in df_dict[key].index
                ):
                    is_subset = False
                    break
...
def runner(data, cfg, counter, batch=64, epochs=100, num_workers=6) -> Tuple[pd.Series]:
    ...
    for key, entry in tmp["val"].iterrows():
        ...
        if data.test_ids & set(entry["Rechecks"]):
            all_rechecks = set(
                entry["Rechecks"]
            )  # - set(data.test_ids) #possible_candidates |  # Possible IDs of rechecks that are not in the test set
            ...
            #     continue
            stones_removed = are_two_stones_a_subset_of_each_other(
                tmp, all_rechecks, second_key
            )
                    ...
                    for idx, xx in enumerate(stones_subset_to_be_considered):
                        for key in client_data.keys():
                            ...
                            if not curr in tmp[key].index:
                                ...
                                new_data_to_add.index = [
                                    curr for _ in range(len(new_data_to_add))
                                ]
                                # if key == "icp":
                                #     breakpoint()
                                # Concatenate the new stone with the old one
                                tmp[key] = pd.concat(
                                    [tmp[key], new_data_to_add], axis=0
                                )
    ...
    if (
        len(data.val_ids) > 50
    ):  # Skip in this filtering when there are not so many stones  (GitHub), since we don't have enough data
        if cfg.target == "origin":
            filtered_df = utils.return_df_for_paper(
                client_data.copy(), "origin", filter_noisy=True
            )
            filtered_val_ids = set(data.val_ids) & set(filtered_df["val"].index)
        elif cfg.target == "treatment":
            filtered_df = utils.return_df_for_paper(
                client_data.copy(), "treatment", filter_noisy=True
            )
            filtered_val_ids = set(data.val_ids) & set(filtered_df["val"].index)
        ...
        filtered_val_ids = data.val_ids

    datamodule_instance = learning.data.DataModule(
        client_data,
        dataset_class,
        cfg,
        number_of_workers=cfg.run.num_workers,
        shuffle_train=True,
        ID_train=data.train_ids,
        ID_val=filtered_val_ids,
        ID_test=data.test_ids,
    )
    ...
    if not "only_val" in cfg or not cfg.only_val.path:
        ...
        folder_path = (
            Path(hydra.utils.to_absolute_path(cfg.only_val.path))
            / f"{counter}"
            / f"lightning_logs/version_0/checkpoints"
        )
    ...
    if cfg.is_consistent_exp != True:
        prob_train, _ = multiple_instances_handler(
            datamodule_instance.train_dataloader, model, cfg
        )
        prob_val, counter = multiple_instances_handler(
            datamodule_instance.val_dataloader, model, cfg
        )
    ...
    prob_test, counter = multiple_instances_handler(
        datamodule_instance.test_dataloader, model, cfg
    )
    ...
    for x_raw, y_raw, _, ID_raw in tqdm(iterator()):
        ...
        for i in range(0, len(ID_raw), 1):
            ...
            if cfg.run.gpu:
                ...
                x = {key: source[i : i + 1] for key, source in x_raw.items()}
