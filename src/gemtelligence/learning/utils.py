...
import pandas as pd
#from ..data_processing import nlp_data
from dataclasses import dataclass
import sklearn
...
class ReadSplit():
    ...
    def split(self,ids_fin,_):
        for i in range(len(self.splits)):
            available = set(self.splits[str(i)]["train"]) | set(self.splits[str(i)]["val"])
...
class CVRes:
    def __init__(self, y_train=None, y_val=None, y_test=None, prob_train=None, prob_val=None, prob_test=None, cfg=None):
        ...
        if prob_train is not None:
            ...
            self.train_cross_entropy = log_loss(y_train, prob_train, labels=[x for x in utils.target_list(cfg.target, cfg.stone_type)])
            
        if prob_val is not None:
            self.val_accuracy = accuracy_score(y_val, np.argmax(prob_val, axis=1))
            print(f"Val Accuracy in CVRes {self.val_accuracy}")
            self.val_cross_entropy = log_loss(y_val, prob_val, labels=[x for x in utils.target_list(cfg.target, cfg.stone_type)])
        ...
        if prob_test is not None:
            ...
            self.test_cross_entropy = log_loss(y_test, prob_test, labels=[x for x in utils.target_list(cfg.target, cfg.stone_type)])
        else:
...
def group_rechecks(df,stone_type, validation_data=[]):
    ...
    total_numer_of_duplicate_stones = sum([len(x) for x in same_stones]) - len(same_stones)
...
def pre_query_correct_stones(curr_dict, cfg):
    ...
    #Preparing heat treatment data
        ...
        curr_df, _ = utils.filter_basaltic_vs_basaltic(curr_df, stone_type=cfg.stone_type)
    else:
    ...
    if cfg.experiments.train_only_on_country:
        ...
        curr_df, _ = curr_df.loc[curr_df["ExcelOrigin"] == cfg.experiments.train_only_on_country], []
    ...
    if cfg.add_secondary_target:
        if cfg.target == "origin":
            ...
            curr_df["Heat Treatment Value"] = curr_df["Heat Treatment Value"].astype('category')
        elif cfg.target == "treatment":
            bool_cond = ~curr_df["FinalOrigin"].isin(utils.target_list("origin", cfg.stone_type))
    ...
    curr_dict["val"] = curr_dict["val"].loc[~curr_dict["val"].index.duplicated(keep='first')]
...
def final_postprocessing(dict_df_client, cfg):
    ...
    for key in dict_df_client.keys():
        if key in ["ed","icp"]:
            dict_df_client[key] = dict_df_client[key].apply(pd.to_numeric,errors="coerce")
...
def cv_training(
    ...
    data.dict_df_client = post_query_correct_stones(data.dict_df_client,list(cfg.sources.keys()))
    ...
    curr_dict = deepcopy(data.dict_df_client) # This is the dict that will be used for training/validation
    ...
    if not cfg.use_fix_generator_path_val: #cfg.use_fix_generator_path:
        ...
        for iii, indices  in enumerate(cross_val_method.split(ids_fin, ori_fin)):
            ...
            splits[iii] = {"train": [int(x) for x in train_ids], "test": [int(x) for x in val_test_ids]}
    ...
    for source in cfg.sources.keys():
        ...
        print("Total number of stones for {} {}".format(source, len(total_stones_per_method[source])))
    ...
    for train_index, test_index in cross_val_method.split(ids_fin, ori_fin):
        ...
        if split_val_test:
            ...
            ori_train.index, ori_train.values, stratify=ori_train, shuffle=True, random_state=2, test_size=0.25)
            
            if not cfg.use_fix_generator_path_val:
                splits[counter] = {"train": [int(x) for x in train_ids], "val": [int(x) for x in val_ids], "test": [int(x) for x in test_ids]}
                ...
                if set(cfg.sources.keys()) == {"ftir","icp","uv","ed"}:   
                    assert [x==y for x,y in zip(sorted(train_ids),sorted(train_ids_candidate))]
                    assert [x==y for x,y in zip(sorted(val_ids),sorted(val_ids_candidate))]
                    assert [x==y for x,y in zip(sorted(test_ids),sorted(test_ids_candidate))]
        ...
        tmp = set(train_ids) & set(val_dict.loc[val_dict[("Considered_Training")] == True].index) #- set(ori_val_test.index)
        ...
        data.train_ids, data.val_ids, data.test_ids = D_train.index, set(val_ids), set(test_ids)
        ...
        elif cfg.method.model_type == "traditional" and (cfg.method.name == "ensemble" or cfg.method.name == "linearlayer"):
            # Traditional method uses sciki-learn (Used for the publication only)
            prob_train, prob_val, prob_test = scikit_learn_interface.ensemble_runner(data, cfg,counter)
        ...
        curr = CVRes(
            y_train=y_train, y_val=y_val, y_test=y_test, prob_train=prob_train_np, prob_val=prob_val_np, prob_test=prob_test_np, cfg=cfg
        )
